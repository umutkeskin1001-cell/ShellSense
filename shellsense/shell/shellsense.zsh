#!/usr/bin/env zsh
# ShellSense — Offline terminal autocomplete for Zsh

if [[ -z "${_SHELLSENSE_LOADED:-}" ]]; then
    _SHELLSENSE_LOADED=1

    if command -v shellsense &>/dev/null; then
        _SHELLSENSE_BIN="shellsense"
    elif [[ -x "$HOME/.shellsense/bin/shellsense" ]]; then
        _SHELLSENSE_BIN="$HOME/.shellsense/bin/shellsense"
    elif [[ -x "$HOME/.cargo/bin/shellsense" ]]; then
        _SHELLSENSE_BIN="$HOME/.cargo/bin/shellsense"
    else
        echo "[shellsense] Binary not found. Run: cargo install --path /path/to/shellsense"
    fi

    if [[ -n "${_SHELLSENSE_BIN:-}" ]]; then
        _SHELLSENSE_CMD_1AGO=""
        _SHELLSENSE_CMD_2AGO=""
        _SHELLSENSE_LAST_CMD=""
        _SHELLSENSE_SUGGESTION=""
        _SHELLSENSE_IS_CORRECTION=0
        _SHELLSENSE_LAST_HIGHLIGHT=""

        if [[ -n "${SHELLSENSE_DATA_DIR:-}" ]]; then
            _SHELLSENSE_SOCK="$SHELLSENSE_DATA_DIR/daemon.sock"
        else
            _SHELLSENSE_SOCK="$HOME/.shellsense/daemon.sock"
        fi

        _shellsense_json_escape() {
            local value="$1"
            value="${value//\\/\\\\}"
            value="${value//\"/\\\"}"
            value="${value//$'\n'/\\n}"
            value="${value//$'\r'/\\r}"
            value="${value//$'\t'/\\t}"
            printf '%s' "$value"
        }

        _shellsense_ensure_daemon() {
            "$_SHELLSENSE_BIN" ping >/dev/null 2>&1 && return 0
            "$_SHELLSENSE_BIN" daemon >/dev/null 2>&1 &
        }

        _shellsense_remove_highlight() {
            if [[ -n "$_SHELLSENSE_LAST_HIGHLIGHT" ]]; then
                region_highlight=("${(@)region_highlight:#$_SHELLSENSE_LAST_HIGHLIGHT}")
                _SHELLSENSE_LAST_HIGHLIGHT=""
            fi
        }

        _shellsense_refresh_highlight() {
            _shellsense_remove_highlight

            if [[ -z "$POSTDISPLAY" ]]; then
                return
            fi

            local color="fg=8"
            if [[ "$_SHELLSENSE_IS_CORRECTION" = "1" ]]; then
                color="fg=3"
            fi

            _SHELLSENSE_LAST_HIGHLIGHT="$#BUFFER $(( $#BUFFER + $#POSTDISPLAY )) $color"
            region_highlight+=("$_SHELLSENSE_LAST_HIGHLIGHT")
        }

        _shellsense_preexec() {
            _SHELLSENSE_LAST_CMD="$1"
        }

        _shellsense_precmd() {
            [[ -z "$_SHELLSENSE_LAST_CMD" ]] && return
            command -v nc >/dev/null 2>&1 || return

            _shellsense_ensure_daemon

            local cmd_esc
            cmd_esc="$(_shellsense_json_escape "$_SHELLSENSE_LAST_CMD")"
            local dir_esc
            dir_esc="$(_shellsense_json_escape "$PWD")"
            local json="{\"Add\":{\"cmd\":\"$cmd_esc\",\"dir\":\"$dir_esc\""

            if [[ -n "$_SHELLSENSE_CMD_1AGO" ]]; then
                local prev_esc
                prev_esc="$(_shellsense_json_escape "$_SHELLSENSE_CMD_1AGO")"
                json+=",\"prev\":\"$prev_esc\""
            fi
            if [[ -n "$_SHELLSENSE_CMD_2AGO" ]]; then
                local prev2_esc
                prev2_esc="$(_shellsense_json_escape "$_SHELLSENSE_CMD_2AGO")"
                json+=",\"prev2\":\"$prev2_esc\""
            fi

            json+="}}"
            print -r -- "$json" | nc -U "$_SHELLSENSE_SOCK" >/dev/null 2>&1 &

            _SHELLSENSE_CMD_2AGO="$_SHELLSENSE_CMD_1AGO"
            _SHELLSENSE_CMD_1AGO="$_SHELLSENSE_LAST_CMD"
            _SHELLSENSE_LAST_CMD=""
        }

        _shellsense_suggest() {
            local prefix="$BUFFER"
            if [[ ${#prefix} -lt 2 ]]; then
                _shellsense_clear
                return
            fi

            command -v nc >/dev/null 2>&1 || return

            local pfx_esc
            pfx_esc="$(_shellsense_json_escape "$prefix")"
            local dir_esc
            dir_esc="$(_shellsense_json_escape "$PWD")"
            local json="{\"Suggest\":{\"prefix\":\"$pfx_esc\",\"dir\":\"$dir_esc\",\"count\":1,\"plain\":true"

            if [[ -n "$_SHELLSENSE_CMD_1AGO" ]]; then
                local prev_esc
                prev_esc="$(_shellsense_json_escape "$_SHELLSENSE_CMD_1AGO")"
                json+=",\"prev\":\"$prev_esc\""
            fi
            if [[ -n "$_SHELLSENSE_CMD_2AGO" ]]; then
                local prev2_esc
                prev2_esc="$(_shellsense_json_escape "$_SHELLSENSE_CMD_2AGO")"
                json+=",\"prev2\":\"$prev2_esc\""
            fi

            local has_env=0
            if [[ -n "$VIRTUAL_ENV" || -n "$KUBECONFIG" || -n "$AWS_PROFILE" ]]; then
                json+=",\"env\":["
                [[ -n "$VIRTUAL_ENV" ]] && json+="\"VIRTUAL_ENV\"" && has_env=1
                if [[ -n "$KUBECONFIG" ]]; then
                    [[ $has_env -eq 1 ]] && json+=","
                    json+="\"KUBECONFIG\""
                    has_env=1
                fi
                if [[ -n "$AWS_PROFILE" ]]; then
                    [[ $has_env -eq 1 ]] && json+=","
                    json+="\"AWS_PROFILE\""
                fi
                json+="]"
            fi
            json+="}}"

            local raw
            raw=$(print -r -- "$json" | nc -U "$_SHELLSENSE_SOCK" 2>/dev/null)
            if [[ -z "$raw" ]]; then
                _shellsense_ensure_daemon
                raw=$(print -r -- "$json" | nc -U "$_SHELLSENSE_SOCK" 2>/dev/null)
            fi

            if [[ -z "$raw" || "$raw" == '{"Error"'* ]]; then
                _shellsense_clear
                return
            fi

            _SHELLSENSE_SUGGESTION="${raw%%$'\n'*}"
            _shellsense_show "$prefix"
        }

        _shellsense_clear() {
            POSTDISPLAY=""
            _SHELLSENSE_SUGGESTION=""
            _SHELLSENSE_IS_CORRECTION=0
            _shellsense_refresh_highlight
        }

        _shellsense_show() {
            local prefix="$1"
            local suggestion="$_SHELLSENSE_SUGGESTION"

            if [[ -z "$suggestion" || "$suggestion" = "$prefix" ]]; then
                _shellsense_clear
                return
            fi

            if [[ "$suggestion" = "$prefix"* ]]; then
                POSTDISPLAY="${suggestion:${#prefix}}"
                _SHELLSENSE_IS_CORRECTION=0
            else
                POSTDISPLAY="  > $suggestion"
                _SHELLSENSE_IS_CORRECTION=1
            fi

            _shellsense_refresh_highlight
        }

        _shellsense_accept() {
            [[ -z "$POSTDISPLAY" ]] && return

            if [[ "$_SHELLSENSE_IS_CORRECTION" = "1" ]]; then
                BUFFER="${POSTDISPLAY#  > }"
            else
                BUFFER="$BUFFER$POSTDISPLAY"
            fi

            CURSOR=${#BUFFER}
            _shellsense_clear
        }

        _shellsense_accept_word() {
            if [[ -z "$POSTDISPLAY" ]]; then
                zle forward-word
                return
            fi

            if [[ "$_SHELLSENSE_IS_CORRECTION" = "1" ]]; then
                BUFFER="${POSTDISPLAY#  > }"
                CURSOR=${#BUFFER}
                _shellsense_clear
                return
            fi

            if [[ "$POSTDISPLAY" =~ "^([^a-zA-Z0-9_.-]*[a-zA-Z0-9_.-]+|[^a-zA-Z0-9_.-]+)" ]]; then
                local chunk="$MATCH"
                BUFFER="$BUFFER$chunk"
                POSTDISPLAY="${POSTDISPLAY#$chunk}"
            else
                BUFFER="$BUFFER$POSTDISPLAY"
                POSTDISPLAY=""
            fi

            CURSOR=${#BUFFER}
            if [[ -z "$POSTDISPLAY" ]]; then
                _shellsense_clear
            else
                _shellsense_refresh_highlight
            fi
        }

        _shellsense_dismiss() {
            _shellsense_clear
        }

        _shellsense_self_insert() {
            local last_char="${KEYS[-1]}"
            zle .self-insert

            if [[ -n "$POSTDISPLAY" && "$_SHELLSENSE_IS_CORRECTION" != "1" && "$POSTDISPLAY" = "${last_char}"* ]]; then
                POSTDISPLAY="${POSTDISPLAY:1}"
                if [[ -z "$POSTDISPLAY" ]]; then
                    _shellsense_clear
                else
                    _shellsense_refresh_highlight
                fi
                return
            fi

            _shellsense_suggest
        }

        _shellsense_backward_delete() {
            zle .backward-delete-char
            _shellsense_suggest
        }

        _shellsense_accept_or_complete() {
            if [[ -n "$POSTDISPLAY" ]]; then
                _shellsense_accept
            else
                zle expand-or-complete
            fi
        }

        _shellsense_forward_char() {
            if [[ -n "$POSTDISPLAY" ]]; then
                _shellsense_accept
            else
                zle forward-char
            fi
        }

        _shellsense_line_init() {
            _shellsense_clear
        }

        zle -N self-insert _shellsense_self_insert
        zle -N backward-delete-char _shellsense_backward_delete
        zle -N _shellsense_accept
        zle -N _shellsense_accept_word
        zle -N _shellsense_dismiss
        zle -N _shellsense_accept_or_complete
        zle -N _shellsense_forward_char
        zle -N zle-line-init _shellsense_line_init

        bindkey '^I'       _shellsense_accept_or_complete
        bindkey '^E'       _shellsense_forward_char
        bindkey '^[[C'     _shellsense_forward_char
        bindkey '^@'       _shellsense_accept_word
        bindkey '^[[1;5C'  _shellsense_accept_word
        bindkey '\e'       _shellsense_dismiss

        autoload -Uz add-zsh-hook
        add-zsh-hook preexec _shellsense_preexec
        add-zsh-hook precmd _shellsense_precmd

        _shellsense_ensure_daemon
    fi
fi
