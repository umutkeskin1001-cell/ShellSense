...
#[command(
    name = "shellsense",
    about = "🧠 Offline terminal AI autocomplete — learns your command patterns",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    /// Output shell integration script or initialize config
    Init {
        ...
        Commands::Init { shell } => {
            match shell.as_deref() {
                ...
                None => {
                    ...
                    // Also create default config if not present
...
exclude_patterns = ["*password*", "*secret*", "*token*", "*api_key*", "*AWS_SECRET*"]
"#;
                        match std::fs::write(&config_path, default_config) {
                            Ok(_) => println!("\n✅ Config created at: {}", config_path.display()),
                            Err(e) => {
            ...
            if let Some(home) = dirs::home_dir() {
                ...
                if zshrc.exists() {
                    if let Ok(content) = std::fs::read_to_string(&zshrc) {
                        if content.contains("shellsense") {
                            let new_content: String = content
                                .lines()
                                .filter(|l| !l.contains("shellsense init zsh") && !l.contains("shellsense.zsh") && !l.contains("ShellSense"))
                                .map(|l| format!("{}\n", l))
                                .collect();
                ...
                if bashrc.exists() {
                    if let Ok(content) = std::fs::read_to_string(&bashrc) {
                        if content.contains("shellsense") {
                            let new_content: String = content
                                .lines()
                                .filter(|l| !l.contains("shellsense init bash") && !l.contains("shellsense.bash") && !l.contains("ShellSense"))
                                .map(|l| format!("{}\n", l))
                                .collect();
                ...
                if fish_config.exists() {
                    if let Ok(content) = std::fs::read_to_string(&fish_config) {
                        if content.contains("shellsense") {
                            let new_content: String = content
                                .lines()
                                .filter(|l| !l.contains("shellsense init fish") && !l.contains("shellsense.fish") && !l.contains("ShellSense"))
                                .map(|l| format!("{}\n", l))
                                .collect();
