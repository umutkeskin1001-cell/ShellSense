...
impl Daemon {
    ...
    pub fn run(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let Some(listener) = self.bind_listener()? else {
            return Ok(());
        };

        // Strict socket permissions (0600)
        if let Ok(mut perms) = std::fs::metadata(&self.socket_path).map(|meta| meta.permissions()) {
            perms.set_mode(0o600);
            if let Err(err) = std::fs::set_permissions(&self.socket_path, perms) {
        ...
        let writer_thread = thread::spawn(move || {
            while let Ok(msg) = rx.recv() {
                match msg {
                    DbMessage::Add { cmd, dir, prev, prev2, timestamp } => {
                        if let Ok(db) = writer_storage.lock() {
                            let _ = db.add_command(
                                &cmd,
                                dir.as_deref(),
                                None,
                                None,
    ...
    fn handle_client(stream: UnixStream, storage: Arc<Mutex<Storage>>, ranker: Arc<Ranker>, tx: Sender<DbMessage>) -> bool {
        ...
        let mut limited = match stream.try_clone() {
            Ok(clone) => clone.take(MAX_REQUEST_SIZE),
            Err(_) => return false,
        };
    ...
    fn bind_listener(&self) -> Result<Option<UnixListener>, Box<dyn std::error::Error>> {
        if Self::socket_has_live_daemon(&self.socket_path) {
            return Ok(None);
        }
    ...
    fn socket_has_live_daemon(path: &Path) -> bool {
        matches!(Self::client_send_to(path, &Request::Ping), Ok(Response::Pong))
    }
    ...
    pub fn client_send(req: &Request) -> Result<Response, String> {
        Self::client_send_to(&Config::socket_path(), req)
    }

    fn client_send_to(path: &Path, req: &Request) -> Result<Response, String> {
...
mod tests {
    ...
    #[test]
    fn test_request_add_accepts_missing_timestamp_and_hour() {
        let json = r#"{"Add":{"cmd":"ls","dir":"/tmp"}}"#;
        let decoded: Request = serde_json::from_str(json).unwrap();
        match decoded {
            Request::Add { timestamp, hour, .. } => {
