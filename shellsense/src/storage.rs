...
impl Storage {
    ...
    fn init_tables(&self) -> Result<()> {
        ...
        if version < 1 {
            ...
            CREATE INDEX IF NOT EXISTS idx_history_dir ON command_history(directory);
            CREATE INDEX IF NOT EXISTS idx_history_ts  ON command_history(timestamp);
            ...
            CREATE TABLE IF NOT EXISTS dir_freq (
            ...
            CREATE TABLE IF NOT EXISTS hour_freq (
            ...
            CREATE TABLE IF NOT EXISTS base_bigrams (
    ...
    pub fn add_command(
        ...
        // Insert into history
        tx.execute(
            "INSERT INTO command_history (command, directory, git_branch, timestamp, exit_code, session_id)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![command, directory, git_branch, timestamp, exit_code, session_id],
        )?;
        ...
        tx.execute(
            "INSERT INTO hour_freq (hour, command, count)
             VALUES (?1, ?2, 1)
             ON CONFLICT(hour, command) DO UPDATE SET count = count + 1",
            params![hour, command],
        )?;
    ...
    /// Get most frequent commands at a specific hour of day
    pub fn get_frequent_by_hour(&self, hour: u32, limit: usize) -> Result<Vec<(String, u32)>> {
        let mut stmt = self.conn.prepare(
            "SELECT command, count FROM hour_freq
             WHERE hour = ?1
             ORDER BY count DESC
             LIMIT ?2"
        )?;
        let rows = stmt.query_map(params![hour, limit as u32], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, u32>(1)?))
        })?;
        rows.collect()
    }
    ...
    pub fn delete_command(&self, cmd: &str) -> Result<()> {
        ...
        tx.execute("DELETE FROM hour_freq WHERE command = ?1", params![cmd])?;
...
mod tests {
    ...
    #[test]
    fn test_hour_frequency() {
        let storage = Storage::open_memory().unwrap();
        let now = chrono::Utc::now().timestamp();

        storage.add_command("good morning", None, None, Some(0), None, now, 9, None, None).unwrap();
        storage.add_command("good morning", None, None, Some(0), None, now, 9, None, None).unwrap();
        storage.add_command("goodnight", None, None, Some(0), None, now, 23, None, None).unwrap();

        let morning = storage.get_frequent_by_hour(9, 10).unwrap();
        assert_eq!(morning[0].0, "good morning");
        assert_eq!(morning[0].1, 2);

        let night = storage.get_frequent_by_hour(23, 10).unwrap();
        assert_eq!(night[0].0, "goodnight");
    }
